...
class Contract_Order_Management_Right_Panel(wx.Panel):
    def __init__(self, parent, log):
        ...
        self.Sizer.Add(wx.Panel(self,-1),3,wx.EXPAND)
        ...
        self.root = self.tree.AddRoot("全部合同")
        ...
        for x in range(15):
            child = self.tree.AppendItem(self.root, "合同 %d" % x)
            self.tree.SetItemData(child, None)
            self.tree.SetItemImage(child, fldridx, wx.TreeItemIcon_Normal)
            self.tree.SetItemImage(child, fldropenidx, wx.TreeItemIcon_Expanded)

            for y in range(5):
                last = self.tree.AppendItem(child, "订单 %d-%s" % (x, chr(ord("a")+y)))
                ...
                for z in range(5):
                    item = self.tree.AppendItem(last,  "组件 %d-%s-%d" % (x, chr(ord("a")+y), z))
